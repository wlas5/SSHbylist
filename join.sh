...
then
	ip=$(cat ~/need/medidores.txt | grep $1 | awk '{ print $3 }') #captura o parâmetro da coluna 3 do arquivo medidores.txt (que corresponde ao ip)
	user=$(cat ~/need/medidores.txt | grep $1 | awk '{ print $1 }') #captura o parâmetro da coluna 1 do arquivo medidores.txt (que corresponde ao usuário)
